...
'required|trim', ['required' => 'Password Harus diisi']);
...
private function _login()
    {
        ...
        $user = $this->ModalUser->cekData(['email'=> $email])->row_array();
        ...
        if($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['passord'])) {
                    ...
                    $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-messsage" role="alert">Password salah!!</div>');
...
public function logout()
    {
        ...
        $this->session->flashdata('pesan','<div class="alert alert-seccess alert-message"role="alert">Anda telah logout!!<div>');
        redirect('Autenfikasi');
    }
}